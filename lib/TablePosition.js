...
class TablePosition extends new Immutable.Record(DEFAULTS) {
     ...
     * @param  {State.Block} startBlock
     * @param  {Object} opts
     * @return {TablePosition}
     */
    static create(state, startBlock, opts) {
        const cell = state.document.getClosest(startBlock.key, (p) => (p.type === opts.typeCell));
        const row = state.document.getClosest(startBlock.key, (p) => (p.type === opts.typeRow));
        const table = state.document.getClosest(startBlock.key, (p) => (p.type === opts.typeTable));
